...
> 本段由 Claude 从分类母版(上层 `AH Apps/CLAUDE.md`)提炼(2026-06-21)。看不到母版的工具(如 Codex)以本段为准;本文件其余部分的项目专属规则优先于本段;细则以母版为准。

- **部署**:GitHub 只存代码,push 不触发部署;改完自验通过即「`commit + push` + 本机 wrangler deploy」一起做(工作区根 `deploy-pages.ps1 -Repo "AH Apps\<repo>"`,自动删私密文件后直传 CF Pages),无需 Oskar 逐次说 deploy。repo 内 deploy.yml 只留手动触发;不开 Cloudflare Git Integration。
- **纯 Worker repo 例外**(wrangler.toml 定义 Worker 入口、无 `pages_build_output_dir`):push 即上线、无 staging——push / deploy 逐次先问 Oskar。
- **栈**:PWA,vanilla HTML/CSS/JS,native ES modules,no build / npm / framework;MSAL.js + Graph → SharePoint;`ah-jobs-rego` 是全套件 job 数据唯一 source of truth,任何 app 不得自建 job list。
